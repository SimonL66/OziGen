...
void writePoint(int fh_qct, long x, long y)
{
	...
	if (nServerType == WMS) {
		...
		OZIGEN_NorthingEasting_to_LatLon(y*nMetres, x*nMetres, &lat, &lon);
	} else {
		OZIGEN_NorthingEasting_to_LatLon(y, x, &lat, &lon);
	}
...
bool CH1903_CreateQCT(LPCSTR mapname, CRect BBox)
{
long x,y;
...
int nMetres = MyMap.GetMetresPerTile();
//double nMetres = 2.6458386250105836 * 256.0;

CTileDatabase db(MyMap.GetDatum(), nMetres);

	CProgressWindow wndProgress;
	wndProgress.Initialize();

	long minx = BBox.left;
	long maxx = BBox.right;
	long miny = BBox.bottom;
	long maxy = BBox.top;
/*
	minx = minx/nMetres;
	miny = miny/nMetres;
	maxx = (maxx+nMetres-1)/nMetres;
	maxy = (maxy+nMetres-1)/nMetres;
*/
...
wndProgress.ResetProgressBar("Shrink1:", (maxy-miny)*(maxx-minx));
	...
	for (y = miny; y < maxy; y++) {
		for (x = minx; x < maxx; x++) {
			squares[(y-miny)*(maxx-minx)+x-minx] = db.TileLoaded(y, x);
...
wndProgress.ResetProgressBar("Shrink2:", (maxy_new-miny_new)*(maxx_new-minx_new));
	...
	if (bitmap_memory == NULL) {
				...
				mm_bitmap_width = (((maxx-minx) * nPixels)+63)/64*64;
				mm_bitmap_height = (((maxy-miny) * nPixels)+63)/64*64;

				OptimizerInit();

				bitmap_offset = 0;
				bitmap_memory_size = (nPixels+64) * mm_bitmap_width;
				...
				square_width = nPixels;
				square_height = nPixels;
				...
				if ((MyMap.GetProjection() == 3812) || (MyMap.GetProjection() == 3857) || (MyMap.GetProjection() == 900913)) {

					// Note: that the OSM map has an inverted y axis.
					CalculateCoefficients(OZIGEN_NorthingEasting_to_LatLon,
										  qct_header.calibration_data,
										  maxy, -(maxy-miny), minx, (maxx-minx),
										  0, (maxx-minx) * nPixels,
										  0, nPixels*(maxy-miny));
				} else {
				...
				// Create outline
				create_outline(fh_qct, minx, miny, maxx, maxy, (maxx-minx) * nPixels, nPixels*(maxy-miny), squares_new);
				free(squares_new);
			}
...
wndProgress.ResetProgressBar("QCT:", (maxy-miny)*(maxx-minx));
	...
	long ystart, yend, ydelta;

	if ((MyMap.GetProjection() == 21781) || (MyMap.GetProjection() == 3812)  || (MyMap.GetProjection() == 900913)) {
		ystart = miny;
		yend = maxy;
		ydelta = 1;
		if (ystart > yend)
...
//	for (y = maxy-1; y >= miny; y--) {
	for (y = ystart; y != yend; y+=ydelta) {
		...
		for (x = minx; x < maxx; x++) {
			...
			(void) CH1903_ReadTiles_QCT(y, x, &db, bitmap_memory + (x-minx)*nPixels, mm_bitmap_width);

		}
		// create row of QCT
		long mmy;
		for (mmy=nPixels+bitmap_offset-64; mmy>=0; mmy-=64) {
			long mmx;
		...
		if (mmy < 0) {
			...
			memmove(bitmap_memory+nPixels*mm_bitmap_width, bitmap_memory, mm_bitmap_width*bitmap_offset);
			memset(bitmap_memory, white_colour, nPixels*mm_bitmap_width);
		}
	}
	...
	if (bitmap_offset != 0) {
		// write final row
		memset(bitmap_memory, white_colour, nPixels*mm_bitmap_width);
		memmove(bitmap_memory+(64-bitmap_offset)*mm_bitmap_width, bitmap_memory+nPixels*mm_bitmap_width, mm_bitmap_width*bitmap_offset);
...
bool OSPro_CreateQCT(LPCSTR mapname, long minx, long miny, long maxx, long maxy)
{
long x,y;
...
wndProgress.ResetProgressBar("Shrink1:", (maxy-miny)*(maxx-minx));
	...
	for (y = miny; y < maxy; y++) {
		for (x = minx; x < maxx; x++) {
			squares[(y-miny)*(maxx-minx)+x-minx] = db.TileLoaded(y*nMetres, x*nMetres);
wndProgress.ProgressBar();
		}
	}
...
long tileX = 0;
long tileY = 0;
if (MyMap.GetProjection() == 21781) {
	tileY = 208;
	tileX = 352;

	tileY = nMetres - (350000 - ((long) 350000 / nMetres * nMetres));
	tileX = nMetres - (420000 - ((long) 420000 / nMetres * nMetres));
}
...
wndProgress.ResetProgressBar("Shrink2:", (maxy_new-miny_new)*(maxx_new-minx_new));
	...
	if (bitmap_memory == NULL) {
				...
				if ((MyMap.GetProjection() == 3857) || (MyMap.GetProjection() == 900913)) {
					...
					CalculateCoefficients(OZIGEN_NorthingEasting_to_LatLon,
										  qct_header.calibration_data,
										  maxy*nMetres, -(maxy-miny)*nMetres, minx*nMetres, (maxx-minx)*nMetres,
										  0, (maxx-minx) * nPixels,
										  0, nPixels*(maxy-miny));
				} else {
					...
					CalculateCoefficients(OZIGEN_NorthingEasting_to_LatLon,
										  qct_header.calibration_data,
										  miny*nMetres-tileY, (maxy-miny)*nMetres, minx*nMetres-tileX, (maxx-minx)*nMetres,
										  0, (maxx-minx) * nPixels,
										  0, nPixels*(maxy-miny));
				}
...
//	for (y = maxy-1; y >= miny; y--) {
	for (y = ystart; y != yend; y+=ydelta) {
		...
		for (x = minx; x < maxx; x++) {
			...
			(void) OSPro_ReadTiles_QCT(y*nMetres, x*nMetres, &db, bitmap_memory + (x-minx)*nPixels, mm_bitmap_width);
...
bool IGN_CreateQCT(LPCSTR mapname, long minx, long miny, long maxx, long maxy)
{
int width, height;
long x,y;
...
bool MakeQCT_main(LPCSTR mapname, CRect BBox)
{
...
/*
	...
	if ((MyMap.GetProjection() == 3857) || (MyMap.GetProjection() == 900913)) {
	 ...
	 return (OSPro_CreateQCT(mapname, minx, ystart, maxx, yend, ydelta));
*/
/*
	minx = minx/1000;
	miny = miny/1000;
...
//		return (IGN_CreateQCT(mapname, minx, miny, maxx, maxy));
		return (OSPro_CreateQCT(mapname, BBox.left, BBox.bottom, BBox.right, BBox.top));
		...
		if (MyMap.GetServerType() == WMS) {
			...
			if ((MyMap.GetProjection() == 2169) || (MyMap.GetProjection() == 21781) || (MyMap.GetProjection() == 3812)
			||  (MyMap.GetProjection() == 900913)
			) {

				return (CH1903_CreateQCT(mapname, BBox));
			}
		}
	}
