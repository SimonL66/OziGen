...
int wmsGetFile(HINTERNET hConnect, LPCSTR pathname, LPCSTR strReferer, LPCSTR tx_saveto)
{
	// Attempt to get file and retry once if failed
	if (wms_get_file_1(hConnect, pathname, strReferer, tx_saveto))
		return 1;
	return wms_get_file_1(hConnect, pathname, strReferer, tx_saveto);
}
...
int nMetres = MyMap.GetMetresPerTile();
	...
	if (wmsGetFile(hConnect, strRequest, strReferer, "tile.tmp")) {
