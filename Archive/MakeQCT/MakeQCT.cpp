...
void CTileDatabase::InitDatabase(long northing, long easting)
{
	...
	sprintf(m_filename, "dbIGN\\%d_%d.db", easting/100000, northing/100000);
...
int IGN_read_bmp(long northing, long easting, CTileDatabase * db, unsigned char * bm_ptr, long bitmap_width)
{
...
long n;
	...
	for (n=0; n<MAX_CACHE_TILES; n++) {
		if (g_TileCache[n*2] == hash) {
			...
			_read(fh_qct, uncompressed+65536, length);
			if (memcmp(data, uncompressed+65536, length) == 0) {
...
{
long tile_length1 = compressHuffman(uncompressed, uncompressed+16384);
long tile_length2 = compress(uncompressed, uncompressed+32768);
__int64 file_offset = _telli64(fh_qct);
unsigned long file_offset_opt;

	if (tile_length1 < tile_length2) {
		if (tile_length1 == 2) {
			if (TileCache00dd[uncompressed[16384+1]]) {
				file_offset = TileCache00dd[uncompressed[16384+1]];
				optimizer1Saved += 2;
				optimizer1Saved_count++;
			} else {
				_write(fh_qct, uncompressed+16384, tile_length1);
				TileCache00dd[uncompressed[16384+1]] = (unsigned long)(file_offset);
			}
		} else {
			if (g_TileCache && (file_offset_opt = OptimizerGetOffset(fh_qct, tile_length1, uncompressed+16384))) {
		...
		if (g_TileCache && (file_offset_opt = OptimizerGetOffset(fh_qct, tile_length2, uncompressed+32768))) {
			file_offset = file_offset_opt;
			optimizer2Saved += tile_length2;
			optimizer2Saved_count++;
		} else {
			_write(fh_qct, uncompressed+32768, tile_length2);
		}
	}
...
void CreateQCT(char * mapname, long minx, long miny, long maxx, long maxy)
{
int width, height;
long x,y;
char * missing_squares;
...
//				}
				...
				for (i=0; i<mm_bitmap_width/64*mm_bitmap_height/64; i++)
	...
	for (y=maxy-1; y>=miny; y--) {
		...
		for (mmy=height+bitmap_offset-64; mmy>=0; mmy-=64) {
			...
			for (mmx=0; mmx<mm_bitmap_width; mmx+=64) {
				long ty;
				for (ty=0; ty<64; ty++)
					memcpy(uncompressed+ty*64, bitmap_memory + (mmy+cvt[ty])*mm_bitmap_width+mmx, 64);
	...
	if (bitmap_offset != 0) {
		...
		for (long mmx=0; mmx<mm_bitmap_width; mmx+=64) {
			for (long ty=0; ty<64; ty++)
				memcpy(uncompressed+ty*64, bitmap_memory + (cvt[ty])*mm_bitmap_width+mmx, 64);
