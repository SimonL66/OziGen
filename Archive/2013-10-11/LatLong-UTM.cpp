...
const double PI       =	4*atan(1.0);	//Gantz used: PI=3.14159265;
...
void LLtoUTM(int eId, double Lat, double Long,  double& Northing, double& Easting, int& Zone){
   ...
   Zone = int((Long + 186)/degrees_per_zone);
   ...
   double LongOrigin = Zone*degrees_per_zone - 183;			//origin in middle of zone
...
void UTMtoLL(int eId, double Northing, double Easting, int Zone,  double* Lat, double* Long){
   ...
   *Lat *= rad2deg;
   *Long = (D-(1+2*T1+C1)*D*D*D/degrees_per_zone+(5-2*C1+28*T1-3*C1*C1+8*EE+24*T1*T1)*D*D*D*D*D/120) / cos(phi1Rad);
