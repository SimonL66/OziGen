...
/*
	...
	case COUNTRY_CAT:
		...
		break;
	case COUNTRY_PT:
		Portugal_WGS84(y*1000, x*1000, &lat, &lon);
		break;
	case COUNTRY_OSM:
		OSM_to_LatLon(y, x, &lat, &lon);
		break;
	case COUNTRY_UK:
...
*/
	OZIGEN_NorthingEasting_to_LatLon(y/**1000*/, x/*1000*/, &lat, &lon);
...
CTileDatabase db(MyMap.GetDatum(), nMeters);
	...
	minx = minx/nMeters;
	miny = miny/nMeters;
	maxx = (maxx+nMeters-1)/nMeters;
	maxy = (maxy+nMeters-1)/nMeters;
...
wndProgress.ResetProgressBar("Shrink1:", (maxy-miny)*(maxx-minx));
	...
	for (y = miny; y < maxy; y++) {
		for (x = minx; x < maxx; x++) {
			squares[(y-miny)*(maxx-minx)+x-minx] = db.TileLoaded(y*nMeters, x*nMeters);
			...
			if (bitmap_memory == NULL) {
				...
				memcpy(&qct_header.palette, mm_osgb_palette, sizeof(qct_header.palette));
					...
					CalculateCoefficients(OZIGEN_NorthingEasting_to_LatLon,
										  ...
										  miny*nMeters, (maxy-miny)*nMeters, minx*nMeters, (maxx-minx)*nMeters,
										  0, (maxx-minx) * nPixels,
										  0, nPixels*(maxy-miny));
/*
				...
				} else if (MyMap.GetCountry() == COUNTRY_ES) {
				...
				} else if (MyMap.GetCountry() == COUNTRY_CZ) {
				...
				} else if (MyMap.GetCountry() == COUNTRY_SI) {
				...
				} else if (MyMap.GetCountry() == COUNTRY_CAT) {
...
wndProgress.ResetProgressBar("QCT:", (maxy-miny)*(maxx-minx));
	...
	for (y = maxy-1; y >= miny; y--) {
		...
		for (x = minx; x < maxx; x++) {
			...
			(void) OSPro_ReadTiles_QCT(y*nMeters, x*nMeters, &db, bitmap_memory + (x-minx)*nPixels, mm_bitmap_width);
...
bool OSM_CreateQCT(LPCSTR mapname, long minx, long miny, long maxx, long maxy)
{
long x,y;
...
wndProgress.ResetProgressBar("Shrink2:", (maxy_new-miny_new)*(maxx_new-minx_new));
	...
	free(squares);
			...
			if (bitmap_memory == NULL) {
				...
				CalculateCoefficients(/*OSM_to_LatLon*/OZIGEN_NorthingEasting_to_LatLon,
									  qct_header.calibration_data,
									  maxy*nMeters, -(maxy-miny)*nMeters, minx*nMeters, (maxx-minx)*nMeters,
									  0, (maxx-minx) * nPixels,
									  0, nPixels*(maxy-miny));
...
*/
	if (MyMap.GetCountry() == COUNTRY_FR) {
		...
		return (OSM_CreateQCT(mapname, minx, miny, maxx, maxy));

	} else {
		
		 return (OSPro_CreateQCT(mapname, minx, miny, maxx, maxy));
	}
