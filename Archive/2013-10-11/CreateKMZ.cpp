...
bool CreateKMZ(long minx, long maxx, long miny, long maxy, LPCSTR mapname, long tileWidth, long tileHeight, long trackSize, int nDatabase, int nDrawOrder) // int nProduct, int nCountry, int nDetail)
{
				...
				if (northing < miny || easting < minx || northing > maxy || easting > maxx) {
					...
					*dest_bitmap_offset++ = 255;
					...
					unsigned char * pal = bitmap_palette + 
												4*bitmap_memory[(long)((maxy-northing)/dTileScale)*bitmap_width +
...
bool DecMapKMZ(CString strPath, CString strFile, CRect MyRect, long tileWidth, long tileHeight, int nDatabase, int trackSize, int nDrawOrder)
{
	...
	g_use_TL3 = nDatabase == 	DBASE_TRACKLOGS_3;
