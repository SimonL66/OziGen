...
long nBits, maxCopies, nCount;
	...
	for (index = 1; index<4096; index++) {
		if (colour != data[index] || nCount == maxCopies) {
			output[len_output++] = (unsigned char)((nCount)<<nBits) + colours[colour];
			colour = data[index];
			nCount = 1;
		} else
			nCount++;
...
unsigned char MAKEQCT_buffer_in[4096];
unsigned char MAKEQCT_buffer_out[8192];
...
FILE * fp;
	for (nColours = 1; nColours<129; nColours++) {
		for (index = 0; index<4096; index++) {
			MAKEQCT_buffer_in[index] = (unsigned char)(rand()%nColours);
		}
		if (nColours == 128) {
			memset(MAKEQCT_buffer_in, 0, sizeof(MAKEQCT_buffer_in));
			fp = fopen("compress.cpp", "r");
			fread(MAKEQCT_buffer_in, 1, 4096, fp);
			fclose(fp);
		}
		length = compress(MAKEQCT_buffer_in, MAKEQCT_buffer_out);
		printf("%d=%d ", nColours, length);

		memset(memory, 0xaa, sizeof(memory));
		uncompressed = memory;
		compressed = MAKEQCT_buffer_out;
		uncompressed_length = 4096;
		decodePositive();
		if (memcmp(memory, MAKEQCT_buffer_in, 4096) != 0 ||
			memory[4096] != 0xaa ||
			memory[4097] != 0xaa ||
			memory[4098] != 0xaa)
		{
		...
		length = compressHuffman(MAKEQCT_buffer_in, MAKEQCT_buffer_out);
		printf("%d\n", length);
		memset(memory, 0xaa, sizeof(memory));
		uncompressed = memory;
		compressed = MAKEQCT_buffer_out;
