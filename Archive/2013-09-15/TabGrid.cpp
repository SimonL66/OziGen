...
bool ExtractGridRef(CString strGridRef, long& nEasting, long& nNorthing, int nCoordinateSystem, bool bLatLonSelected, CString& strErrorMsg)
{
	if (bLatLonSelected) {
		...
		if (nCoordinateSystem == COORD_OSGB)							LatLon_to_NorthingEasting(dLat, dLon, &dNorthing, &dEasting);

		else if (nCoordinateSystem == COORD_SCOTLAND)					LatLon_to_NorthingEasting(dLat, dLon, &dNorthing, &dEasting);

		else if (nCoordinateSystem == COORD_AUSTRIA)					WGS84_Austria(dLat, dLon, dNorthing, dEasting);
		else if (nCoordinateSystem == COORD_FRANCE_LAMBERT_1993)		WGS84ToLambert93(dLat, dLon, &dNorthing, &dEasting);
		else if (nCoordinateSystem == COORD_GERMANY_GAUSS_KRUEGER)		WGS84_Germany(dLat, dLon, dNorthing, dEasting);
		else if (nCoordinateSystem == COORD_SPAIN_ED50)					WGS84_Spain(dLat, dLon, dNorthing, dEasting);
		else if (nCoordinateSystem == COORD_CZECH)						WGS84_Czech(dLat, dLon, dNorthing, dEasting);
		else if (nCoordinateSystem == COORD_SLOVENIA)					WGS84_Slovenia(dLat, dLon, dNorthing, dEasting);
		else if (nCoordinateSystem == COORD_CATALONIA)					WGS84_Catalonia(dLat, dLon, dNorthing, dEasting);

		else if (nCoordinateSystem == COORD_BELGIUM_LAMBERT_2008)		WGS84ToLambertBelgium2008(dLat, dLon, &dNorthing, &dEasting);
		
		else {
			strErrorMsg.Format("Error: Unknown Coordinate System for %s",strGridRef);
			return false;			
		}
		...
		if (!ExtractEastingNorthing(strGridRef, nEasting, nNorthing)) {
			strErrorMsg.Format("Error: Invalid Lambert08 Coord %s",strGridRef);
			return false;
		}
	}

	if ((nCoordinateSystem == COORD_FRANCE_LAMBERT_1993) && !IsInIGNDatabase(nNorthing, nEasting)) {
		strErrorMsg.Format("Error: the map for %s is not supported in the IGN Database",strGridRef);
		return false;
	}

	if ((nCoordinateSystem == COORD_OSGB) && !IsInOSGBDatabase(nNorthing, nEasting)) {
...
pParent = (COziGenDlg *)(this->GetParent()->GetParent());
	...
	if (!ExtractGridRef(m_strGridRef, easting, northing, g_CoordinateSystem, bLatLonSelected, strErrorMessage))
	{
...
void CTabGrid::UpdateGridExample()
{
	...
	if (pParent->m_nCountry == COUNTRY_UK) {
	...
	} else if (pParent->m_nCountry == COUNTRY_FR) {
	...
	} else if (pParent->m_nCountry == COUNTRY_BE) {
		if (pParent->m_nCoordSystem == COORD_SYSTEM_GRID)		m_strGridExample.Format("e.g. Lambert 2008: 650000, 671000");
		if (pParent->m_nCoordSystem == COORD_SYSTEM_LAT_LON)	m_strGridExample.Format("e.g. 50.8411° N, 4.3564° E");
	} else if (pParent->m_nCountry == COUNTRY_DE) {
		if (pParent->m_nCoordSystem == COORD_SYSTEM_GRID)		m_strGridExample.Format("e.g. Gauss Krueger: 3798000 5831000");
		if (pParent->m_nCoordSystem == COORD_SYSTEM_LAT_LON)	m_strGridExample.Format("e.g. 52.5167° N, 13.3833° E");
	} else if (pParent->m_nCountry == COUNTRY_ES) {
		if (pParent->m_nCoordSystem == COORD_SYSTEM_GRID)		m_strGridExample.Format("e.g. ED50: 443000 4473000");
		if (pParent->m_nCoordSystem == COORD_SYSTEM_LAT_LON)	m_strGridExample.Format("e.g. 40.4000° N, 3.6833° W");
	} else if (pParent->m_nCountry == COUNTRY_CZ) {
		if (pParent->m_nCoordSystem == COORD_SYSTEM_GRID)		m_strGridExample.Format("e.g. UTM 33: 459000 5549000");
		if (pParent->m_nCoordSystem == COORD_SYSTEM_LAT_LON)	m_strGridExample.Format("e.g. 50.0833° N, 14.4167° E");
	} else {
...
pParent = (COziGenDlg *)(this->GetParent()->GetParent());
	...
	if ((g_CoordinateSystem == COORD_OSGB) || (g_CoordinateSystem == COORD_SCOTLAND))  {
