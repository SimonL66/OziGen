...
bool Create_OZI_PNG(LPCSTR mapname, long minx, long maxx, long miny, long maxy, int nDatabase, int nFlavour, int nCountry, int nScale)
{
...
*/
	...
	if (nDatabase == DBASE_LOCAL) {
		...
		if ((nCountry == COUNTRY_OSGB) || (nCountry == COUNTRY_NGI) || (nCountry == COUNTRY_ES)) {
			...
			successful = OSPro_ReadTiles(minx, maxx, miny, maxy, wndProgress, 
										...
										nFlavour, nCountry, nScale);
/*
			successful = Kompass_ReadTiles(minx, maxx, miny, maxy, wndProgress, 
											bitmap_palette, bitmap_palette_length, 
											bitmap_memory , bitmap_memory_size, 
											bitmap_width, dest_bitmap_width,
											bitmap_height, dest_bitmap_height,
