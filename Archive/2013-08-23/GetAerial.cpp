...
private:
	...
	struct {
		...
		long	m_scale;
		DWORD	m_padding[4];
		DWORD	m_offsets[100*2*100*2];		// 0=unused; 1=doesn't exist; else tile offset => DWORD length, data
	} m_fileheader;
...
{
	if (m_hFile != INVALID_HANDLE_VALUE) {
		// check for tile in same database
	...
	if (m_hFile == INVALID_HANDLE_VALUE) {
		...
		if (m_hFile == INVALID_HANDLE_VALUE) {
			...
			m_fileheader.m_magic='J' + ('P'<<8)+ ('G'<<16)+ ('d'<<24);
	...
	if (m_fileheader.m_magic != ('J' + ('P'<<8)+ ('G'<<16)+ ('d'<<24)) ||
		m_fileheader.m_northing != northing/100000 ||
		m_fileheader.m_easting != easting/100000 ||
		m_fileheader.m_scale != 500)
	{
...
long x_value;
	...
	if (colour_convert[x_value] == COLOUR_UNKNOWN)
		colour_convert[x_value] = RGBtoPalette(ap25_nz_qct_palette, 128, r, g, b);
...
int read_jpg(long northing, long easting, CTileDatabase * db, char * szFile, unsigned char * bm_ptr, long bitmap_width)
{
	...
	if (bmp.bmWidth != TILE_WIDTH || bmp.bmHeight != TILE_WIDTH) {
	...
	for (y=0; y < TILE_WIDTH; y++) {
		for (x=0; x<TILE_WIDTH; x++) {
			bm_ptr[x+(TILE_WIDTH-y-1)*bitmap_width] = get_colour((unsigned char*)bmp.bmBits+y*bmp.bmWidthBytes+x*3);
		}
	}
...
void Create_PNG_JPR_from_JPEG(char * mapname, long minx, long maxx, long miny, long maxy)
{
char tstring[256];
...
unsigned char * bitmap_memory = (unsigned char *)malloc(bitmap_size);
	...
	for (int y=maxy; y>miny; y-=500) {
		...
		for (int x=minx; x<maxx; x+=500) {
			...
			printf("%s\r", tstring);
			read_jpg(y, x, &g_db, tstring, bitmap_memory+(x-minx)/500*TILE_WIDTH, bitmap_width);
		}
...
int get_file(HINTERNET hConnect, LPCSTR pathname, long northing, long easting, CTileDatabase * db)
{
HINTERNET hReq;
DWORD  dwSize, dwCode;
CHAR szData[HTTP_GET_SIZE+1];
	...
	if (memcmp(buffer, jpghdr, sizeof(jpghdr)) == 0) {
		...
		free(buffer);
		return get_file(hConnect, pathname, northing, easting, db);
	}
	free(buffer);
	return TRUE;
}
...
int LoadTiles(long minx, long miny, long maxx, long maxy)
{
...
char tstring[1024];
	...
	for (int x=minx; x<maxx; x+=500) {
		for (int y=maxy; y>miny; y-=500) {
			...
			easting_northing_to_OSref3(x, y, tstring);
			sprintf(tstring, "/webpreview/preview.aspx?x=%d&y=%d&productID=9&dataSet=mm&size=500", 250+x, y-252);

			if (get_file(hConnect, tstring, y, x, &g_db)) {
...
// GetAerial [extent1] [extent2]
// GetAerial [minx] [maxx] [miny] [maxy]
...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	if (!AfxWinInit(::GetModuleHandle(NULL), NULL, ::GetCommandLine(), 0)) {
		...
		if (argc == 3) {
			...
			printf(	"GetAerial [-s] extent1 extent2\n"
					"GetAerial [-s] easting1 easting2 northing1 northing2\n"
		...
		Create_PNG_JPR_from_JPEG("map", minx, maxx, miny, maxy);
		free(colour_convert);
