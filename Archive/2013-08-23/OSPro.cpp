			...
			for (int Yoffset=0; Yoffset < MyMap.m_nTilesPerKilometre; Yoffset ++) {
				for (int Xoffset=0; Xoffset < MyMap.m_nTilesPerKilometre; Xoffset ++) {
					if (MyMap.m_nTileBitDepth == 8) {
						...
						if (MyMap.m_nCountry == COUNTRY_DE) {
							...
							read_png_10K(y+MyMap.m_nMetresPerTile*Yoffset, 
										 x+MyMap.m_nMetresPerTile*Xoffset, 
										 &db,
										 bitmap_offset+(MyMap.m_nTilesPerKilometre-1-Yoffset)*MyMap.m_nPixelsPerTile*bitmap_width+MyMap.m_nPixelsPerTile*Xoffset,
										 bitmap_width);
...
/*
	if (nMapScale == MAP_SCALE_50K) {
		read_png(y*1000, x*1000, db, bitmap_offset + 0*bitmap_width, bitmap_width);
		tilewidth = 200;
...
*/
	...
	for (y=0; y < MyMap.m_nTileWidth/2; y++) {
		for (x=0; x < MyMap.m_nTileWidth; x++) {
			...
			bitmap_offset[x+y*bitmap_width] = palette_convert[bitmap_offset[x+(MyMap.m_nTileWidth-1-y)*bitmap_width]];
			bitmap_offset[x+(MyMap.m_nTileWidth-1-y)*bitmap_width] = t;
		}
	}
	return true;
}