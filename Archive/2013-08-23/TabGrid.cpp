...
//	pParent->m_nUTMZone = 0;
	...
	if (g_CoordinateSystem == COORD_OSGB) {
		...
		if (pParent->m_nCoordSystem == COORD_SYSTEM_LAT_LON) {
			...
			WGS84toGaussKrueger(dLat, dLon, d_northing, d_easting);
		...
		if (pParent->m_nCoordSystem == COORD_SYSTEM_LAT_LON) {
			...
			WGS84toED50(dLat, dLon, d_northing, d_easting);
			...
			if (!ExtractGrid(m_strGridRef, easting, northing)) {
				pParent->AddToResultsWindow("Error: Invalid German Coord %s",m_strGridRef);
				return;
			}
		}
	}
