...
void writePoint(int fh_qct, long x, long y)
{
	...
	case COORD_FRANCE_LAMBERT_1993:
		...
		break;
	case COORD_SPAIN:
		ED50toWGS84(y*1000, x*1000, &lat, &lon);
		break;
	case COORD_GERMANY_GAUSS_KRUEGER:
		GaussKruegertoWGS84(y*1000, x*1000, &lat, &lon);
		break;
	case COORD_OSGB:
	default:
...
//										(maxy-miny) * height);
				...
				if (MyMap.m_nCountry == COUNTRY_OSGB) {
				...
				} else if (MyMap.m_nCountry == COUNTRY_NGI) {
					...
					CalculateCoefficients(ED50toWGS84,
										  qct_header.calibration_data,
										  miny*1000, (maxy-miny)*1000, minx*1000, (maxx-minx)*1000,
										  0, (maxx-minx) * width,
										  0, height*(maxy-miny));
				} else if (MyMap.m_nCountry == COUNTRY_DE) {
					CalculateCoefficients(GaussKruegertoWGS84,
										  qct_header.calibration_data,
										  miny*1000, (maxy-miny)*1000, minx*1000, (maxx-minx)*1000,
										  0, (maxx-minx) * width,
										  0, height*(maxy-miny));
...
if (g_CoordinateSystem == COORD_FRANCE_LAMBERT_1993) {
	...
	if ((g_CoordinateSystem == COORD_OSGB) || (g_CoordinateSystem == COORD_SPAIN)) {
