...
void CTabMap::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_LONG_NAME, m_bLongMapName);
}
...
pParent = (COziGenDlg *)(this->GetParent()->GetParent());
	...
	if (m_bLongMapName) {
		...
		pParent->m_strMapName = strMap + "_" + RemoveSpaces(m_strMapName);
		
	} else {
		pParent->m_strMapName = strMap;
	}

	pParent->m_nCountry = COUNTRY_OSGB;
