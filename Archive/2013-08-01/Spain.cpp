...
int Spain_abortProgram = FALSE;

BOOL Spain_CtrlCHandler(DWORD dwCtrlType)
{
	Spain_abortProgram = TRUE;
	return TRUE;
}
...
BOOL Spain_ErrorOut(DWORD dError, TCHAR * szCallFunc)
{
...
int Spain_get_file_1(HINTERNET hConnect, LPCSTR pathname, LPCSTR tx_saveto)
{
HINTERNET hReq;
DWORD  dwSize, dwCode;
CHAR szData[HTTP_GET_SIZE+1];
	...
	if ( !(hReq = HttpOpenRequest (hConnect, "GET", pathname, HTTP_VERSION, "", NULL, 0 ,0 ))) {
		Spain_ErrorOut (GetLastError(), "HttpOpenRequest");
		return FALSE;
	}


	if (!HttpSendRequest (hReq, NULL, 0, NULL, 0) ) {
		Spain_ErrorOut (GetLastError(), "HttpSend");
		return FALSE;
	}

	dwSize = sizeof (DWORD) ;
	if ( !HttpQueryInfo (hReq, HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER, &dwCode, &dwSize, NULL)) {
		Spain_ErrorOut (GetLastError(), "HttpQueryInfo");
		return FALSE;
	}
	...
	long file_len=0;
	while (!Spain_abortProgram) {
		if (!InternetReadFile (hReq, (LPVOID)szData, HTTP_GET_SIZE, &dwSize) ) {
			Spain_ErrorOut (GetLastError (), "InternetReadFile");
			file_len = -1;
			break;
		}
		if (dwSize == 0)
...
//		printf("%d \r", file_len);
	...
	if (!InternetCloseHandle (hReq) ) {
		Spain_ErrorOut (GetLastError (), "CloseHandle on hReq");
		file_len = -1;
	}
	if (file_len <= 0)
		return FALSE;
...
int Spain_get_file(HINTERNET hConnect, LPCSTR pathname, LPCSTR tx_saveto)
{
// Attempt to get file and retry once if failed
	if (Spain_get_file_1(hConnect, pathname, tx_saveto))
		return 1;
	return Spain_get_file_1(hConnect, pathname, tx_saveto);
}
...
//	sprintf(strRequest, "osmapapi/ts?FORMAT=image%%2Fpng&KEY=DEMOAPI&URL=http%%3A%%2F%%2Fosopenspacewiki.ordnancesurvey.co.uk%%2Fwiki%%2Fextensions%%2Fhtmlets%%2FOSProCS.html&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&STYLES=&EXCEPTIONS=application%%2Fvnd.ogc.se_inimage&LAYERS=5&PRODUCT=50K&SRS=EPSG%%3A27700&"
	...
	if (Spain_get_file(hConnect, strRequest, "tile.tmp")) {
...
g_db->InitDatabase(y,x,MAP_DATUM_ES);
	...
	if (hOpen != NULL) {
		if (!InternetCloseHandle (hConnect) ) {
			Spain_ErrorOut (GetLastError (), "CloseHandle on hConnect");
			return FALSE;
		}
		if (!InternetCloseHandle (hOpen) ) {
			Spain_ErrorOut (GetLastError (), "CloseHandle on hOpen");
			return FALSE;
		}
	}
	return 0;
