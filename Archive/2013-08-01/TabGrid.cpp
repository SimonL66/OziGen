...
pParent = (COziGenDlg *)(this->GetParent()->GetParent());
	...
	} else if (g_CoordinateSystem == COORD_KOMPASS) {
...
//		nMax = (northing+999)/1000*1000;
	...
	CString str;
	str.Format("map%s_%s_%d_%d",GetMapScaleText(pParent->m_nScale),GetMapFlavourText(pParent->m_nFlavour),easting,northing);
