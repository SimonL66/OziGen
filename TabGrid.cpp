...
double tmpLat, tempLon;
OZIGEN_NorthingEasting_to_LatLon(dNorthing, dEasting, &tmpLat, &tempLon);
...
//	long eMin, nMin, eMax, nMax;
	...
	if (MyMap.GetServerType() == WMS) {
		...
		MyRect.left = MyRect.left/nMetres*nMetres;
		MyRect.top = (MyRect.top+nMetres-1)/nMetres*nMetres;
		MyRect.right = (MyRect.right+nMetres-1)/nMetres*nMetres;
		MyRect.bottom = MyRect.bottom/nMetres*nMetres;

	} else {

		if ((MyMap.GetProjection() == 2169) 
		||  (MyMap.GetProjection() == 3812) 
		||  (MyMap.GetProjection() == 21781) 
		||  (MyMap.GetProjection() == 3857) 
		||  (MyMap.GetProjection() == 900913)) {
...
/*
	str.Format("%s%s_%s_%d_%d",
		MyMap.GetInternetCode(),MyMap.GetMapScaleText(pParent->m_nLayer),MyMap.GetMapFlavourText(pParent->m_nCountry,pParent->m_nProduct),easting,northing);
*/
